...
def test_required_does_not_affect_property_identity(tmp_path):
    ...
    (it lives on Rule instead — see test_registry_property_does_not_retain_
...
def test_identical_rule_from_two_sources_shares_one_hash_id(tmp_path):
    ...
    insert_schema(conn, parse_linkml(FIXTURES / "required_a.yml"), "required_a", agent="tester")
    insert_schema(conn, parse_linkml(FIXTURES / "required_c.yml"), "required_c", agent="tester")
